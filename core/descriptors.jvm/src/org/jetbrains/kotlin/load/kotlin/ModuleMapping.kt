...
import org.jetbrains.kotlin.serialization.jvm.ModuleProtoBuf
    ...
    companion object {
        ...
        fun create(
            ...
            if (configuration.skipMetadataVersionCheck || version.isCompatible()) {
                val moduleProto = ModuleProtoBuf.Module.parseFrom(stream) ?: return EMPTY
...
class PackageParts(val packageFqName: String) {
    ...
    fun addTo(builder: ModuleProtoBuf.Module.Builder) {
        if (parts.isNotEmpty()) {
            builder.addPackageParts(ModuleProtoBuf.PackageParts.newBuilder().apply {
        ...
        if (metadataParts.isNotEmpty()) {
            builder.addMetadataParts(ModuleProtoBuf.PackageParts.newBuilder().apply {
                packageFqName = this@PackageParts.packageFqName
    ...
    private fun ModuleProtoBuf.PackageParts.Builder.writePartsWithinPackage(parts: List<String>) {
    ...
    private fun ModuleProtoBuf.PackageParts.Builder.writePartsOutsidePackage(
            parts: List<String>,
            packageTableBuilder: ModuleProtoBuf.Module.Builder
    ) {
